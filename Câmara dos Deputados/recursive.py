...
def reqDiscursos(deputado: int, idLegislatura: list = [], dataInicio: str = "", dataFim: str = "", ordenarPor: str = "dataHoraInicio", ordem: str = "ASC") -> list:
    ...
    for i in resps:
        try:
            lista_discursos.extend(i.json()["dados"]) # Tratar o fato de que pode haver corpos vazios
        except:
            pass
    lista_discursos = {str(deputado): lista_discursos}
...
def reqMembros(idPartido: int, dataInicio: str = "", dataFim: str = "", idLegislatura: list = [], ordenarPor: str = "", ordem: str = "ASC") -> list:
    ...
    for i in resps:
        for j in i.json()["dados"]:
            lista_deputados.append(int(j["id"]))
    lista_discursos_deputados = []
    for deputado in lista_deputados:
        lista_discursos_deputados.append(reqDiscursos(deputado=deputado, idLegislatura=idLegislatura, dataInicio=dataInicio, dataFim=dataFim, ordenarPor=ordenarPor, ordem=ordem))
        time.sleep(5)
...
def reqPartidos(siglas: list = [], dataInicio: str = "", dataFim: str = "", idLegislatura: list = [], ordem: str = "ASC", ordenarPor: str = "sigla", ordenarPorDiscursos: str = ""):
    ...
    for Id in ids:
        lista_discursos_deputados_partidos[Id] = reqMembros(idPartido=Id, dataInicio=dataInicio, dataFim=dataFim, idLegislatura=idLegislatura, ordenarPor=ordenarPorDiscursos, ordem=ordem)
